...
VulkanShader::VulkanShader( VulkanDevice& device, std::string const& shaderPath) : 
	vulkanDevice{device}, shaderPath{shaderPath}
{
...
VulkanPipelinee::VulkanPipelinee(
		VulkanDevice& device,
	...
	vulkanDevice(device)
{
	...
	this->createPipeline(renderPass, vertexShaderFile, fragmentShaderFile, modelType, textureType);
}
...
void VulkanPipelinee::bind(VkCommandBuffer commandBuffer)
{
	vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, this->pipeline);
}

void VulkanPipelinee::createPipelineLayout(std::vector<VkDescriptorSetLayout> const& descriptorSetLayouts)
{
...
void VulkanPipelinee::createPipeline(
	VkRenderPass renderPass,
	std::string const& vertexShaderFile,
	std::string const& fragmentShaderFile,
	ModelType modelType,
...
)
{
	...
	VulkanShader vertexShader{this->vulkanDevice, vertexShaderFile};
	VulkanShader fragmentShader{this->vulkanDevice, fragmentShaderFile};
	...
	VulkanPipelineConfig pipelineConfig = this->defaultPipelineConfigInfo(renderPass, textureType);
...
VulkanPipelineConfig VulkanPipelinee::defaultPipelineConfigInfo(VkRenderPass renderPass, TextureType textureType) const noexcept
{
	assert(this->pipelineLayout != nullptr && "Cannot create pipeline before pipeline layout");
	...
	} else if (textureType == TEXTURE_PLAIN) {
